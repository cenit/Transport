      SUBROUTINE ELIPS3(NNR,NRJ,SCALX,SCALY,ISIZE,COLF)
C
...
C
      ...
      DIMENSION   PXMAX(LDM),PYMAX(LDM),R13(LDM),R24(LDM)
C
...
C
      ...
      CALL PSTEXT((OFFX1+5.)*SCALX,(ENDY1+10.)*SCALY,'Y')
      CALL PSTEXT((ENDX2+5.)*SCALX,OFFY1*SCALY,'X''')
...
C
      ...
      WRITE(TEXT,1006) CHIX, R13(J)
 1006 FORMAT('Chi1=',F7.1,' Deg',' (',F7.4,')')
      CALL PSTEXT(BEGX1*SCALX,(ENDY1+85.)*SCALY,TEXT)

      WRITE(TEXT,1007) CHIY, R24(J)
 1007 FORMAT('Chi2=',F7.1,' Deg',' (',F7.4,')')
      CALL PSTEXT(BEGX2*SCALX,(ENDY1+85.)*SCALY,TEXT)

      WRITE(TEXT,1008) EX, DIM2, DIM2
 1008 FORMAT('Eps1=',F6.2,' * Pi ',A2,A2)
      CALL PSTEXT(BEGX1*SCALX,(ENDY1+50.)*SCALY,TEXT)

      WRITE(TEXT,1009) EY
 1009 FORMAT('Eps2=',F9.2,' * Pi mrmr')
      CALL PSTEXT(BEGX2*SCALX,(ENDY1+50.)*SCALY,TEXT)

      WRITE(TEXT,1013) XSCAL,DIM2,PXSCAL, DIM2
 1013 FORMAT('Xmax=',F5.1,1X,A2,'  Ymax=',F5.1,1X,A2)
      CALL PSTEXT(BEGX1*SCALX,(BEGY1-30.)*SCALY,TEXT)

      WRITE(TEXT,1012) YSCAL,PYSCAL
 1012 FORMAT('X''max=',F5.1,' mr','  Y''max=',F5.1,' mr')
