      SUBROUTINE DTMW(IJK,XMIN,XMAX,IYU,LTR,XSCAL,YSCAL)
C
...
C
      ...
      AUX=SIG*MAT(IND)
      Y(II,L)=AUX
      IF (IV .EQ. 1) YMAX=MAX(ABS(AUX),YMAX)
 20   CONTINUE
      II=II+1
      GO TO 1
 100  M=II-1
...
C
      IF (IV .EQ. 1) THEN
       ...
       CALL TEXT_OUT(EXP//char(0),870.*XSCAL,705.*YSCAL,1)
      ENDIF
C
...
C
      ...
      DO 120 L=1, N
       ...
       DO 110 II=J1,J2,J3
        ...
        IF (II .EQ. J1) CALL GRAMOV(X(J1),YY)
        CALL GRADRW(X(II),YY)
         IF (L .EQ. 1 .AND. N .GT. 1) THEN
          CALL CROSS(X(II),YY,SIZE)
         ELSE IF (L .EQ. 2) THEN
          CALL TRIUP(X(II),YY,SIZE)
         ELSE IF (L .EQ. 3) THEN
          CALL TRIDO(X(II),YY,SIZE)
         ELSE
          CALL SQUARE(X(II),YY,SIZE)
         ENDIF
      CALL GRAMOV(X(II),YY)
 110  CONTINUE
      CALL REVERSE(LTR)
...
C
      ...
      YYU=YU+SIZE/2.
      XXU=(950.-85.*FLOAT(N))*XSCAL
      DO 150 L=1, N
      XXU=XXU+85.*XSCAL
      INF(L)(1:1)='T'
      IF (L .EQ. 1 .AND. N .GT. 1) THEN
       CALL SET_COL(1)
       CALL CROSS(XXU,YYU,SIZE)
       CALL TEXT_OUT(INF(L)//char(0),XXU+15.*XSCAL,YU,1)
      ELSE IF (L .EQ. 2) THEN
       CALL SET_COL(2)
       CALL TRIUP(XXU,YYU,SIZE)
       CALL TEXT_OUT(INF(L)//char(0),XXU+15.*XSCAL,YU,1)
      ELSE IF (L .EQ. 3) THEN
       CALL SET_COL(3)
       CALL TRIDO(XXU,YYU,SIZE)
       CALL TEXT_OUT(INF(L)//char(0),XXU+15.*XSCAL,YU,1)
      ELSE IF (L .EQ. 4) THEN
       CALL SET_COL(5)
       CALL SQUARE(XXU,YYU,SIZE)
       CALL TEXT_OUT(INF(L)//char(0),XXU+15.*XSCAL,YU,1)
      ELSE
      END IF
 150  CONTINUE
...
C
 ...
 1020 FORMAT(6X,F6.2,9X,F6.2)
      ...
      SUBROUTINE CROSS(X,Y,SIZE)
      CALL GRAMOV(X-SIZE/2.,Y+SIZE/2.)
      CALL GRADRW(X+SIZE/2.,Y-SIZE/2.)
      CALL GRAMOV(X-SIZE/2.,Y-SIZE/2.)
      CALL GRADRW(X+SIZE/2.,Y+SIZE/2.)
      RETURN
      END

      SUBROUTINE TRIUP(X,Y,SIZE)
      CALL GRAMOV(X,Y+SIZE/2.)
      CALL GRADRW(X+SIZE/2.,Y-SIZE/2.)
      CALL GRADRW(X-SIZE/2.,Y-SIZE/2.)
      CALL GRADRW(X,Y+SIZE/2.)
      RETURN
      END

      SUBROUTINE TRIDO(X,Y,SIZE)
      CALL GRAMOV(X,Y-SIZE/2.)
      CALL GRADRW(X+SIZE/2.,Y+SIZE/2.)
      CALL GRADRW(X-SIZE/2.,Y+SIZE/2.)
      CALL GRADRW(X,Y-SIZE/2.)
