      SUBROUTINE ELIW3(SCALXX,SCALYY)
C
C   PLOTS BEAM ELLIPSES
C   ADAPTED FOR WORKSTATION-UIS BY U.ROHRER (PSI), JANUARY 1989
C       For (x,y and x',y') space by U. Rohrer (PSI), June. 2005
C
      PARAMETER (LDM=2000)
      CHARACTER*80 TEXT
      CHARACTER*20 INPUT
     ...
     *  CHI(50),IPOINT(50),IREM(50),FAIL(50)
      ...
      COMMON /EP/ Z,XMAX,YMAX,PXMAX,PYMAX,R13,R24,LAB,DIM1,DIM2
C
...
C
      IF (INPUT(1:1).EQ.'L' .OR. INPUT(1:1).EQ.'l') THEN
       CALL VT220_POP
       CALL ELIPS3(NNR,NRJ,SCALX/SCALXX,SCALY/SCALYY,SIZE,.FALSE.)
...
C
     ...
     1     (INPUT(2:2).NE.'O') ) THEN
       write(6,'(A)') INPUT(2:2)
       CALL VT220_POP
       CALL ELIPS3(NNR,NRJ,SCALX/SCALXX,SCALY/SCALYY,SIZE,.TRUE.)
...
C
      ...
      Y1M=200.*YMAX(J)/YSCAL
      CH1=ASIN(R13(J))

      X2M=200.*PXMAX(J)/PXSCAL
...
C   EMITTANZ
C
      EX=XMAX(J)*YMAX(J)*SQRT(1.-R13(J)**2)
      EY=PXMAX(J)*PYMAX(J)*SQRT(1.-R24(J)**2)
C
C   WINKEL CHIX & CHIY
C
      CHIX=180./PI*ASIN(R13(J))
      CHIY=180./PI*ASIN(R24(J))
C
C   ZEICHNE ELLIPSEN
C
      OFFX1 = OFFX(NJ)
...
C
      ...
      CALL TEXT_OUT('Y'//char(0),(OFFX1+5.)*SCALX,
     1                           (ENDY1+10.)*SCALY,SIZE)
      CALL TEXT_OUT('X'''//char(0),(ENDX2+5.)*SCALX,OFFY1*SCALY,SIZE)
...
C
      ...
      WRITE(TEXT,1006) CHIX, R13(J), char(0)
 1006 FORMAT('Chi1=',F7.1,' Deg',' (',F7.4,')',A1)
      CALL TEXT_OUT(TEXT,BEGT1*SCALX,(ENDY1+85.)*SCALY,SIZE)

      WRITE(TEXT,1007) CHIY, R24(J), char(0)
 1007 FORMAT('Chi2=',F7.1,' Deg',' (',F7.4,')',A1)
      CALL TEXT_OUT(TEXT,BEGT2*SCALX,(ENDY1+85.)*SCALY,SIZE)

      WRITE(TEXT,1008) EX, DIM2, DIM2, char(0)
 1008 FORMAT('Eps1=',F6.2,' * Pi ',A2,A2,A1)
      CALL TEXT_OUT(TEXT,BEGT1*SCALX,(ENDY1+50.)*SCALY,SIZE)

      WRITE(TEXT,1009) EY, char(0)
 1009 FORMAT('Eps2=',F9.2,' * Pi mrmr',A1)
      CALL TEXT_OUT(TEXT,BEGT2*SCALX,(ENDY1+50.)*SCALY,SIZE)

      WRITE(TEXT,1013) XSCAL,DIM2,YSCAL,DIM2,char(0)
 1013 FORMAT('Xmax=',F5.1,1X,A2,'  Ymax=',F5.1,1X,A2,A1)
      CALL TEXT_OUT(TEXT,BEGT1*SCALX,(BEGY1-30.)*SCALY,SIZE)

      WRITE(TEXT,1012) PXSCAL,PYSCAL,char(0)
 1012 FORMAT('X''max=',F5.1,' mr','  Y''max=',F5.1,' mr',A1)
...
C
      ...
      END
C
      INTEGER FUNCTION IFONTSIZE3(SCALX)
C
      IFONTSIZE3 = NINT(5.0 * SCALX - 1.0)
      IF (IFONTSIZE3 .LT. 0) IFONTSIZE3 = 0
      IF (IFONTSIZE3 .GT. 3) IFONTSIZE3 = 3
C
      RETURN
      END
