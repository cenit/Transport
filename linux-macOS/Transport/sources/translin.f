...
C
C     Note:: (NOV. 1997, LINUX Version only)
C     Names: REPEAT replaced by REPEAT1 and UPDATE replaced by UPDATE1
C
...
C SUBROUTINE REPEAT1(DONE) KEEPS TRACK OF TYPE CODE 9 (REPEAT ELEMENT).
...
C SUBROUTINE UPDATE1 RESETS FLAGS SO THAT ACCUMMULATION OF THE R1 MATRIX
C                  WILL BEGIN ANEW.
C SUBROUTINE UPDAT2 SAME AS UPDATE1, BUT FOR R2 MATRIX.
C SUBROUTINE UPDAT3 SAME AS UPDATE1, BUT FOR R3 MATRIX.
...
C
  600 IF(SLITS) GO TO 5003
      ...
      IF (K .EQ. 2) GO TO 620
      GO TO 5003
  610 IF (RCP .OR. R2P) CALL UPDATE1
      GO TO 5003
...
C
      CHARACTER TIT*10
      CHARACTER    FILNAM*10, ANS*3, COMM*6, RM*17
      REAL DUM(6)
      LOGICAL FIRST, FIRSTS, FX
      DATA FIRST, FIRSTS /.TRUE., .TRUE./
      DATA FILNAM /'FOR002.DAT'/
      DATA PI /3.141592654/
C
      IF (.NOT.FIRST) GO TO 5
      FIRST = .FALSE.
      INQUIRE(FILE=FILNAM,EXIST=FX)
      IF (FX) THEN
       INQUIRE(FILE=FILNAM,FORMATTED=ANS)
       IF (ANS(1:3) .EQ. 'YES') THEN
        COMM = 'rm -f '
        RM = COMM//FILNAM//char(0)
        CALL SYSTEM(RM)
       ENDIF
      ENDIF
      OPEN(UNIT=NOUT+1,FILE=FILNAM,FORM='UNFORMATTED',
     X     STATUS='UNKNOWN')
      REWIND NOUT+1
      WRITE(NOUT+1) (XDIME(J),J=1,11), AMP
 1002 FORMAT(12A4)
c
c     FILE FOR FLOOR LAYOUT COORDINATES OUTPUT IN PLOT MODE
c
      OPEN(UNIT=29,FILE='FOR029.DAT',STATUS='UNKNOWN')
      REWIND 29
      WRITE(29,9998)
 9998 FORMAT('                        l             ',
     A       'x         y         z',
     B       '             theta       phi')  
C
    5 IF (.NOT. (FIRSTS .AND. SEC) ) GO TO 10
      FIRSTS = .FALSE.
      OPEN(UNIT=0,FILE='FOR000.DAT',FORM='UNFORMATTED',
     X     STATUS='UNKNOWN')
      REWIND 0
      WRITE(0) (XDIME(J),J=1,11)
 1003 FORMAT(11A4)
C
   10 CONTINUE
...
C     MOST ELEMENTS
C
   20 WRITE(NOUT+1) TYPE, LABEL(NUM), (DATA(I+J), J=1,4), ALONG,
     A              (CENUN(J),J=1,6), (SIUN(J),J=1,6),R12,R34,R56,
     B              R13,R24,(RMTX(J),J=1,13)
 1001 FORMAT(34A4)
      GO TO 5000
C
C     1. -- BEAM
C
  100 WORK = RI / UNIT(11)
      WRITE(NOUT+1) TYPE, LABEL(NUM), WORK, (DUM(J),J=1,3), ALONG,
     A              (CENUN(J),J=1,6), (SIUN(J),J=1,6),R12,R34,R56,
     B              R13,R24,(RMTX(J),J=1,13)
      GO TO 5000
C
...
C
  200 IF (DATA(I+1) .GE. 1000.) THEN
       WORK = DATA(I+1)
       WRITE(NOUT+1) TYPE, LABEL(NUM), WORK, XI1, XI2, XI3, ALONG,
     A               (CENUN(J),J=1,6), (SIUN(J),J=1,6),R12,R34,R56,
     B               R13,R24,(RMTX(J),J=1,13)
       GO TO 5000
      ELSE
...
C
  400 CONTINUE
      ...
      WORK = AL / UNIT(7)
      BNORM = B / UNIT(9)
      WRITE(NOUT+1) TYPE, LABEL(NUM), DATA(I+1), BNORM, NB, WORK,
     A              ALONG,(CENUN(J),J=1,6), (SIUN(J),J=1,6),
     B              R12, R34, R56, R13, R24, (RMTX(J),J=1,13)
      GO TO 5000
  401 CONTINUE
      WORK = DATA(I+2)
      BNORM = FIELD
...
C
  600 IF (ISTOR(NUM+1)-I .EQ. 5) THEN
       WRITE(NOUT+1) TYPE, LABEL(NUM), (DATA(I+J), J=1,4), ALONG,
     A               (CENUN(J),J=1,6), (SIUN(J),J=1,6),R12,R34,R56,
     B               R13,R24,(RMTX(J),J=1,13)
      ELSE
       XN1 = 0.
       XN2 = 0.
       WRITE(NOUT+1) TYPE, LABEL(NUM), (DATA(I+J), J=1,2),
...
C
 1000 Z = LC / UNIT(8)
      WRITE(NOUT+1) TYPE, LABEL(NUM), (DATA(I+J), J=1,4), COC, Z,
...
C
 1100 WORK1 = RI / UNIT(11)
      WORK2 = RI * WORK1 / (SM * ( 1. + GAMMA ))
      WRITE(NOUT+1) TYPE, LABEL(NUM), DATA(I+1), WORK2, WORK1,
     A              DUM(1), ALONG,
     B              (CENUN(J),J=1,6), (SIUN(J),J=1,6),R12,R34,R56,
     C              R13,R24,(RMTX(J),J=1,13)
      GO TO 5000
C
C     FLOOR LAYOUT COORDINATES
C
 5000 IF (LAY) THEN
       CALL GETTIT(TYPE,TIT)
       THETA = ATAN(O(4,3,1) / O(4,3,3))
       IF (O(4,3,3) .GE. 0.0) GO TO 72
       SHIFT = SIGN(PI,O(4,3,1))
       THETA = SHIFT - THETA
   72  THETA = THETA / UNIT(7)
       ...
       WRITE(29,9999) TIT, LABEL(NUM),ALONG,XDIME(8),
     A                X0(4,1)/UNIT(8),X0(4,2)/UNIT(8),X0(4,3)/UNIT(8),
     B                XDIME(8),THETA,PHI,XDIME(7)
 9999  FORMAT(A10,' /',A4,'/',F10.3,' ',A4,3F10.3,' ',A4,
     A        2F10.3,' ',A4)
      ENDIF
C
...
C
     ...
     A    TYPE .NE. 19 .AND. TYPE .NE. 20 .AND. TYPE .NE. 21) RETURN
      WRITE(0) TYPE,ALONG,LABEL(NUM),TOTROT,(SRMTX(J),J=1,105)
 2000 FORMAT(109A4)
C
      RETURN
      END
...
C     UNDEFINED OPERATIONS
C
  900 WRITE(NOUT,*) 'SPESHL: UNDEFINED CODE.'
      RETURN
      END
...
C
      LOGICAL   FX
      CHARACTER COMM*80, ANS*3, FILNAM*10, RM*80
      DATA      FILNAM /'FOR002.DAT'/
C
      CALL GETARG(1,COMM)
C
      IF (COMM(1:1) .EQ. '1') THEN
       FORMIO = .TRUE.
      ELSE
       FORMIO = .FALSE.
      ENDIF
...
C
      ...
      IF (FX) THEN
       INQUIRE(FILE=FILNAM,FORMATTED=ANS)
       IF (ANS(1:2) .EQ. 'NO') THEN
        COMM(1:6) = 'rm -f '
        RM(1:17) = COMM(1:6)//FILNAM//char(0)
        CALL SYSTEM(RM)
       ENDIF
      ENDIF
C
      IF (FORMIO) THEN
       OPEN(UNIT=NOUT,FILE=FILNAM,STATUS='UNKNOWN')
       REWIND NOUT
       CALL ABOUT
      ENDIF
C
...
C
      WRITE(NOUT,*) ' '
      WRITE(NOUT,1000)
 1000 FORMAT('TRANS computational part for Transport',
     X     ' for Linux, compiled by U. Rohrer (PSI), 8-Dec-2009')
      WRITE(NOUT,*) ' '
C
      RETURN
      END
