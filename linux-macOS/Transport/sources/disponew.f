...
C       REDESIGNED FOR Tcl/Tk controlled output (U.R.Apr.2005)
C
...
C
    DATA TEXT/'z   ','x   ','x''  ','r12 ','y   ','y''  ',  
     X  'r34 ','R16 ','R26 ','p   ','dp/p','epsx','epsy'/
C
    DATA ITEM/'ZMIN ','ZMAX ','XMPL ','YMPL ','APFAC'
     X  ,'SPEZ ','DISP ','YDISP','RAX  ','RAY  ','MEASX'
     X  ,'MEASY','MEASD','CUMAX','SPEZ2','CSX  ','CSY  '
     X  ,'YTIT ','STEER'/
...
C
    CALL GETARG(1,KEY)
1111    WRITE(6,3011) UEBER(1:NLEN)
    WRITE(6,*) ' '
    CALL TOUPPER(KEY)
...
C
   ...
   98   CLOSE(UNIT=29)
    ...
    ENDIF
    IF (PRINT) THEN
     PRINT *, 'COORDS.DAT written to print_file OUTPUT.DAT.'
        ENDIF
    IF (I .EQ. 0) THEN
     WRITE(IO,1020)
     GO TO 999
...
C
    ...
    ENDIF
    IF (PRINT) THEN
     PRINT *, 'BEAM.DAT written to print_file OUTPUT.DAT.'
        ENDIF
    GO TO 1
C
C   QUADS
C
  200   IF (FIRST) THEN
    ...
    5    CONTINUE
     ...
     READ(50,2077,END=6)
     X   QTYP(K),QLAB(K),LEFF(K),JA(K),A0(K),A1(K),A2(K),A3(K),
     X   J0(K),JMAX(K),DMAX(K)
     K = K + 1
     GO TO 5
    6    NREC = K - 1
     CLOSE(UNIT=50)
...
C
    IF (KEY.EQ.'QUADS' .OR. KEY.EQ.'Q') THEN
     ...
     ENDIF
     IF (PRINT) THEN
      PRINT *, 'QUADS.DAT written to print_file OUTPUT.DAT.'
         ENDIF
    ENDIF
C
    IF (K.EQ.0) GO TO 1
    IF (KEY.EQ.'QUADS' .OR. KEY.EQ.'Q') GOTO 1
c   IF (KEY.EQ.'QUADS' .OR. KEY.EQ.'Q') THEN
c    WRITE(6,1030) ESC,ESC,ESC
c    WRITE(6,3014) ESC,ESC
c    READ(5,1000,END=1111) KEY
c    CALL TOUPPER(KEY)
c   ENDIF
...
C
    ...
    ENDIF
    IF (PRINT) THEN
     PRINT *, 'QDAC.DAT written to print_file OUTPUT.DAT.'
        ENDIF
    GO TO 1
C
C   QUAD AMPS
...
C
    ...
    ENDIF
    IF (PRINT) THEN
     PRINT *, 'QAMP.DAT written to print_file OUTPUT.DAT.'
        ENDIF
    GO TO 1
C
C   BENDS
...
C
  ...
  251   READ(11,END=253,ERR=253)
    ...
    I = I + 1
    IF(TITEL.EQ.'*BEND*')
     X  WRITE(IO,4006) TITEL,LABEL,XL,DIM1,AP,DIM2,B,DIM3,XN,ANG,DIM6
    IF(TITEL.EQ.'*E-PRISM*')
     X  WRITE(IO,4007) TITEL,LABEL,XL,DIM1,AP,DIM2,B,XN,ANG,DIM6
    IF(TITEL.EQ.'*ROTAT*')
     X  WRITE(IO,3006) TITEL,LABEL,XL,DIM6,AP,B,XN
    GO TO 251
...
C
    ...
    ENDIF
    IF (PRINT) THEN
     PRINT *, 'BENDS.DAT written to print_file OUTPUT.DAT.'
        ENDIF
    GO TO 1
C
C   BEAMLINE
...
C
    ...
    ENDIF
    IF (PRINT) THEN
     PRINT *, 'BEAMLINE.DAT written to print_file OUTPUT.DAT.'
        ENDIF
C
    IF (I .EQ. 0) THEN
     WRITE(IO,1020)
...
C
    ...
    ENDIF
    IF (PRINT) THEN
     PRINT *, 'FITS.DAT written to print_file OUTPUT.DAT.'
        ENDIF
    GO TO 1
C
...
C
 1402   MAL = MAL + 1
    ...
    IF (FIRST2) THEN
     ...
     X            INDMAX,IND,IN,IS,ICH,CHI,IPOINT,SAV1,SAV2,SAV3
     READ(15) (TT(I),LB(I),T1(I),T2(I),T4(I),T5(I),
     X      U1(I),U2(I),U3(I),U4(I),U5(I),U6(I),U7(I),U8(I),
     X      V1(I),V2(I),V3(I),V4(I),V5(I),V6(I),V7(I),V8(I),V9(I),
     X      W1(I),W2(I),W3(I),W4(I),W5(I),W6(I),W7(I),I=1,INDMAX)
...
C
    ...
    ENDIF
    IF (PRINT) THEN
     PRINT *, 'SHIFTS.DAT written to print_file OUTPUT.DAT.'
        ENDIF
    GO TO 1
C
C   DISPLAY
...
C
    ...
    ENDIF
    IF (PRINT) THEN
     PRINT *, 'DISPLAY.DAT written to print_file OUTPUT.DAT.'
        ENDIF
    GO TO 1
C
...
c
    write(6,5558)
        READ(5,1234,END=9998) INPUT
    CALL TOUPPER(INPUT)
c
    IF(INPUT(1:1) .EQ. 'P') THEN
         PRINT = .TRUE.
    ...
    ENDIF
c
    IF(INPUT(1:1) .EQ. 'F') THEN
     FILE  = .TRUE.
     PRINT *,'requested transport data will be stored'
     OPEN(UNIT=7,FILE='OUTPUT.DAT',STATUS='UNKNOWN')
     REWIND(UNIT=7)
     WRITE(7,3012) UEBER
     IO = 7
...
c3011   FORMAT(' ',A1,'[0;0H',A1,'[0m',A,A1,'[0m')
 3011   FORMAT(A)
 3012   FORMAT(/,1X,A79,//)
 ...
 5558   FORMAT('P(rint), F(ile) or Enter for exit ',$)
    END
