...
C
C     MODIFIED FOR DIGITAL VISUAL FORTRAN (VERSION 5.0)
C     BY URS ROHRER (PSI) IN JULY 1997
C
...
C ---------------------------------------------------------------------
C STOP                FLAG EXCHANGE SBR SUPPIO -> SBR MENU
C NOSTOP       LOGICAL   TRUE FOR NO ERROR HALT IN SBR MENU
C ---------------------------------------------------------------------
C ---------------------------------------------------------------------
C ---------------------------------------------------------------------
C
...
C
      CHARACTER TIT*10
      CHARACTER    FILNAM*10, ANS*3, COMMAND*80
      LOGICAL RESULT
      REAL DUM(6)
...
C
      ...
      IF (FX) THEN
       ...
       COMMAND = 'del '//FILNAM//char(0)
c       IF (ANS(1:3) .EQ. 'YES') CALL REMOVE(FILNAM//char(0))
       IF (ANS(1:3) .EQ. 'YES') RESULT = SYSTEMQQ(COMMAND)
      ENDIF
...
C
      LOGICAL   FX, RESULT
      CHARACTER COMM*80, ANS*3, FILNAM*10
      DATA      FILNAM /'FOR002.DAT'/
C
      CALL GETARG(1,COMM,ISTATUS)
C
      IF (ISTATUS .NE. -1) THEN
       IF (COMM(2:2) .EQ. '1') THEN
        FORMIO = .TRUE.
        NOSTOP = .FALSE.
       ELSE IF (COMM(2:2) .EQ. '2') THEN
        FORMIO = .TRUE.
        NOSTOP = .TRUE.
       ELSE
        FORMIO = .FALSE.
...
C
      DO 102 J = 1, LDIR
       IF (DIR(J:J) .EQ. '$') DIR(J:J) = ' '
  102 CONTINUE
C
      INQUIRE(FILE=DIR(1:LDIR)//FILNAM,EXIST=FX)
      IF (FX) THEN
       INQUIRE(FILE=DIR(1:LDIR)//FILNAM,FORMATTED=ANS)
       COMM = 'del '//DIR(1:LDIR)//FILNAM//char(0)
c       IF (ANS(1:2) .EQ. 'NO') CALL REMOVE(DIR(1:LDIR)//FILNAM//char(0))
       IF (ANS(1:2) .EQ. 'NO') RESULT = SYSTEMQQ(COMM)
      ENDIF
C
      IF (FORMIO) THEN
       OPEN(UNIT=NOUT,FILE=DIR(1:LDIR)//FILNAM,STATUS='UNKNOWN')
       REWIND NOUT
       CALL ABOUT
      ENDIF
C
...
c      SEEDVAL = 100 * ISEC + I100TH
c      SEEDVAL = RND$TIMESEED
c      CALL SEED(SEEDVAL)
      ISEED(1) = SECNDS(0.0)
      M=1
...
C
      WRITE(NOUT,*) ' '
      WRITE(NOUT,1000)
 1000 FORMAT('TransNT.exe computational part for Transport',
