...
C
      IF (MOD(NCT,100) .EQ. 1) THEN
       ...
       OPEN(UNIT=567,FILE=DIR(1:LDIR)//'STOPTURT',STATUS='OLD',
     X      ERR=778)
       CLOSE(UNIT=567)
       NORAYS = NCT - 1
       GO TO 699
...
C
   ...
   20 MC = 0
      EMPTY = .FALSE.
      IF (FIRST) THEN
       OPEN(UNIT=NIN,FILE=DIR(1:LDIR)//'FOR001.DAT',STATUS='OLD',
     X      ERR=60)
       FIRST = .FALSE.
      ENDIF
...
C
      IF (IFLAG .EQ. 1) THEN
       OPEN(UNIT=99,FILE=DIR(1:LDIR)//'ERROR.TRA',STATUS='UNKNOWN')
        write(99,*) 'error'
       CLOSE(UNIT=99)
       write(*,1000)
...
C
      WRITE(NOUT,*) ' '
      WRITE(NOUT,1000)
 1000 FORMAT('TurtleNT.exe computational part for Turtle',
