...
C     Recompiled for Windows NT in April 1997 by Urs Rohrer (PSI)
C     Graphics is now Done via CernLib's HBOOK/HPLOT package
C     Recompiled for Windows NT in June 1998 for DEC-Fortran Version 6
C     Extended to function as a thread controlled via file-input, which
C     is generated from a C++ class, February 2001 by U. Rohrer (PSI).
C
      include 'flib.fi'
C
     ...
     A                YSTRT(20), YEND(20), LTHY(20), YBIN(20), NUNY(20)
      ...
      logical open
      common /icoord0/ xl0, yl0, xl20, yl20, xl30, yl30
...
C
      ...
      DATA IOUNIT, SCLTYP, TYP /100, 'lin', 'C1'/
      data id, mx, my, open /10, 2, 2, .FALSE./
      data MODE,ICOLH,ICOLB,ICOLP,JCOLH,JCOLB,JCOLP/'M',2,3,5,4,5,7/
      data width, height /19.5,19.5/
      data first2 /.TRUE./
      DATA STAT /.FALSE./
C
...
C
   ...
   98   DIR = COMM(1:J)
       ...
       ENDIF
      ENDIF
      FILENAME = DIR(1:LDIR)//'GRATUR.INP'
      METAFILE = DIR(1:LDIR)//'Metafile.rz'
C
...
C
      ...
      IF (ISTATUS .NE. -1) THEN
       IF (COMM(1:4) .EQ. '1234') THEN
        FROMFILE = .TRUE.
        CALL SYSTEMQQ('DEL '//FILENAME//CHAR(0))
       ENDIF
      ENDIF
C
C     READ DATA ON STORING FILE (FOR100.DAT)
C
      OPEN(UNIT=IOUNIT,ERR=998,FILE=DIR(1:LDIR)//'FOR100.DAT',
     X     STATUS='OLD')
      READ(IOUNIT,1235,err=998) NHISTS,LIMNO,LOSS,NORAYS
 1235 FORMAT(4I10)
      READ(IOUNIT,1236,err=998) (MTABLE(I),I=1,LIMNO)
 1236 FORMAT(10F13.3)
      READ(IOUNIT,1237,err=998) (XMOM(J),J=1,NHISTS)
 1237 FORMAT(10E13.5)
      READ(IOUNIT,1237,err=998) (XMOM2(K),K=1,NHISTS)
      READ(IOUNIT,1236,err=998) (ZPOS(K),K=1,NHISTS)
      READ(IOUNIT,1248,err=998) (XLAB(K),K=1,NHISTS)
 1248 FORMAT(10(1X,A4))
      READ(IOUNIT,1236,err=998) ((ISTOP(K,J),K=1,3),J=1,9998)
      READ(IOUNIT,1238,err=998) UNIT,NAME,NOM,NOM1
 1238 FORMAT(12E13.5,/,12(A4,1X),/,12(A4,1X),/,12(A4,1X))
      READ(IOUNIT,1239,err=998) (MTYPE(I),NENTRY(I),NCOOR(I),
     X   INTVLS(I),BEGIN(I),END(I),STEP(I),ZHIST(I),I=1,NHISTS)
 1239 FORMAT(I2,I6,2I4,4E13.5)
      READ(IOUNIT,1234,err=998) (NBLK(I),KFLG(I),XSTRT(I),
     X XEND(I),LTHX(I),XBIN(I),NUNX(I),YSTRT(I),YEND(I),
     Y LTHY(I),YBIN(I),NUNY(I), I=1,LOSS)
 1234 FORMAT(I6,I2,2E13.5,I4,E13.5,I3,2E13.5,I4,E13.5,I3)
      CLOSE(UNIT=IOUNIT)
C
C     OPEN SPECIAL FILE FOR MENT
C
      OPEN(UNIT=11,FILE=DIR(1:LDIR)//'FOR011.DAT',
     X     STATUS='UNKNOWN')
      REWIND(UNIT=11)
C
      call Header
C
 ...
 2000 FORMAT(/' There are',I3,' histograms stored on FOR100.DAT')
      WRITE(6,2100) NORAYS
 2100 FORMAT(' All booked histograms refer to ',I8,' rays')
      WRITE(6,*) 'and will be shown in a window named HIGZ_01.'
      WRITE(6,*) ' '
C
      IF (FROMFILE) THEN
       WRITE(6,*) 'The input is managed via a Dialog Box.'
       WRITE(6,*) ' '
      ENDIF
C
...
c
   11 CONTINUE
      IF (FROMFILE) THEN
       IND = IWAITFOR(FILENAME,METAFILE)
       IF (IND .EQ. -1) GO TO 999
       read(10,1016,err=1999) x, y
       if (.NOT. FIRST2) go to 22
      ELSE
       write(6,1015) width, height
 1015  format(
...
c
   22 CONTINUE
      IF (FROMFILE) THEN
       read(10,1018,err=1999) m1, m2
       if (.NOT. FIRST2) go to 33
      ELSE
       write(6,1017) mx, my
...
c
 ...
 1019  format(' Fill color indices are:'/
   ...
   33 CONTINUE
      IF (FROMFILE) THEN
       read(10,1022,err=1999) I1,I2,I3
C       if (.not. first2) go to 44
      ELSE
       write(6,1020) ICOLH, ICOLB, ICOLP
 1020  format(' For 1-dim Histograms:'/
   ...
   44 CONTINUE
      IF (FROMFILE) THEN
       read(10,1022,err=1999) J1,J2,J3
C       if (.not. first2) go to 1
      ELSE
       write(6,1021) JCOLH, JCOLB, JCOLP
...
C
  600 ND   = LTHY (K)
      ...
      GO TO 300
C
  998 PRINT *,'FOR100.DAT not found or corrupt, program exits.'
      go to 2001
C
  999 call hplend
C
...
C
      IF (FROMFILE) THEN
       COMM = 'DEL '//DIR(1:LDIR)//'GRATUR.INP'//CHAR(0)
       CALL SYSTEMQQ(COMM)
      ENDIF
      CLOSE(UNIT=11)
      CALL EXIT
...
C
   77 CONTINUE
      IF (FROMFILE) THEN
       READ(10,1003,err=1999) INPUT  	! DUMMY FOR 3D-TYPE
       READ(10,1003,err=1999) INPUT	! COLOR FOR 1-DIM HIST
       READ(10,1001,err=1999) I      	! DUMMY FOR NNIV
 1001  FORMAT(I3)
       READ(10,1004,err=1999) PAR1,PAR2  ! DUMMY FOR PHI, THETA
 1004  FORMAT(2F5.0)
      ELSE
       WRITE(6,1002) MODE
 1002  FORMAT(' Enter',
...
c
 ...
 5000 FORMAT(I3)
      ...
      ENDIF
C
C     OUTPUT TO SPECAIL FILE FOR MENT
C
      WRITE(11,'(A)') TITEL
      WRITE(11,1234) (Y(I)/10., I=1,N)
 1234 FORMAT(10F7.1)
C
...
c      call htitle('Graphic Turtle Output')
      if (MODE .eq. 'C') then
	if (ICOLP .gt. 1107 .or. ICOLB .gt. 1107) then ! P or B greater 7
          call hplset('PCOL',0)
          call hplset('HCOL',ICOLH - 1100)
          call hplset('HTYP',354.)    ! /////
          call hplset('BCOL',0)
	else
          call hplset('PCOL',ICOLP)
          call hplset('HCOL',ICOLH)
          call hplset('BCOL',ICOLB)
	endif
      else
...
c
  ...
  300	CONTINUE
	IF (FROMFILE) THEN
         READ(10,1003,err=1999) INPUT	  ! COLOR FOR 1-DIM HIST
 1003    FORMAT(A3)
         READ(10,1001,err=1999) I          ! NUMBER OF CONTOUR LINES
         READ(10,1004,err=1999) PAR1,PAR2  ! DUMMY FOR PHI, THETA
 1004    FORMAT(2F5.0)
	ELSE
	 WRITE(6,1000) NNIV
...
c
      if (MODE .eq. 'C') then
        call hplset('PCOL',JCOLP)
        call hplset('HCOL',JCOLH)
        call hplset('BCOL',JCOLB)
      else
        call hplset('PCOL',0)       ! mono
        call hplset('HCOL',0)       ! mono
        call hplset('BCOL',0)       ! mono
...
C
   10 CONTINUE
      IF (FROMFILE) THEN
       READ(10,1003,err=1999) INPUT	! COLOR FOR 1-DIM HIST
 1003  FORMAT(A3)
       READ(10,1002,err=1999) I      	! DUMMY FOR NNIV
 1002  FORMAT(I3)
       READ(10,1001,err=1999) PAR1, PAR2 ! PHI AND THETA
      ELSE
       WRITE(6,1000) PHI, TET
...
c	ixgetwi is not (yet) implemented under Windows
c
c        call ixgetwi(1,iwid)           ! get window ID
c        call ixgetge(iwid,ix,iy,iw,ih) ! get width and height in pixels
c        iw = int(iw * width/19.5)
c        ih = int(ih * height/19.5)
c        call ixrscwi(iwid,iw,ih)
...
c
      ...
      xl3 = xl + xs - xoff - 13 * chs - max(0,my-mx) * chs
      yl3 = yl - 2.5 * chs - max(0,mx-my) * chs
      if (mx.eq.1 .and. my.eq.1) yl2 = yl - 0.95 * ys     
c
      return
      end
...
1000  format(/
     &  ' Welcome to the TURTLE Graphic Histogram Option Programme.')
      write(6,1001)
1001  format(
     &  ' Version 1.23 for Windows, using HBOOK/HPLOT from CernLib 98')
      write(6,1002)
1002  format(
     &  ' Compiled with Digital Visual Fortran Version 6.0' )
      write(6,1003)
1003  format(
     &  ' =========================================================')
      write(6,1004)
...
c
      ...
      INTEGER FUNCTION IWAITFOR(FILE,MF)
C
      USE DFLIB
C
      common /ionumb/ id, mx, my, xl, yl, xl2, yl2, xl3, yl3, chs, open
...
C
    1 OPEN(UNIT=10,STATUS='OLD',NAME=FILE,ERR=999)
      REWIND(UNIT=10)
      READ(10,998,ERR=999) NUMB, PRFLAG
 998  FORMAT(I10,A2)
C
      IF (NUMB .EQ. -1) THEN
       CLOSE(UNIT=10) 
      ...
      ELSE
       NUMBOLD = NUMB
       IF (PRFLAG(1:2) .NE. '  ') THEN
	if (PRFLAG(1:1) .eq. 'm') then
	  if (open) then
	   call hrput(0,MF,'N')
	   write(6,*) 'Metafile.rz containing last histogram generated.'
	  endif
	else
          if (open) then
           call ClosePS
           call SYSTEM('printg '//PRFLAG(1:2)//' hist.ps')
           id  = 10   ! --> clears screen
           xl  = xl0  ! --> reset coordinates
           yl  = yl0
           xl2 = xl20
