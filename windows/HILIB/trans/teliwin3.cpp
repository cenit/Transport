///////////////////////////////////////////////////////////////////////////////
//////////////////// class TransEllipseWin3 implementation ////////////////////
///////////////////////////////////////////////////////////////////////////////
...
TransEllipseWin3 :: TransEllipseWin3(int nCmdShow, LPCSTR szWindowName,
...
TransEllipseWin3 :: ~TransEllipseWin3(VOID)
{
    TransEllipseWin3::WmClose();
}

BOOL TransEllipseWin3 :: WmDestroy( void )
{
    return FALSE;
}

BOOL TransEllipseWin3 :: WmClose( void )
{
    if (!closed)
    {
...
long TransEllipseWin3 :: WmSysMenuCommand(WPARAM MenuItem)
{
    switch (MenuItem)
    {
...
long  TransEllipseWin3 :: WmKeyDown(WPARAM Key)
{
...
VOID TransEllipseWin3 :: CreatePens(int w)
{
    if (w != ww)
    {
        if (ww != -1) DestroyPens();
...
VOID TransEllipseWin3 :: DestroyPens(VOID)
{
...
VOID TransEllipseWin3 :: DrawFrame(VOID)
{
    Brush *opOld;
    int   i, j, fs, index;
    float finc;
...
//  Set Font
    ...
    {  GraMove(x0x[i]-len,y0x[i]); GraDraw(x0x[i]+len,y0x[i]);
       ...
       PutString(x0x[i],y0x[i]-len-fs-3*fs/4,"y");
       ...
       PutString(x0y[i]+len+fs/2,y0y[i]-fs,"x'");
...
VOID TransEllipseWin3 :: Scaling(float p, float& scal, int& length,
...
long TransEllipseWin3 :: WmEraseBackGround(void)
{
    return FALSE; // tell Windows to not erase background (white flickering)
//    return DefaultEvent();
}

long TransEllipseWin3 :: WmSize(int w, int h)
{
    h_user = h;
    w_user = w;
...
long TransEllipseWin3 :: WmPaint( const PAINTSTRUCT & )
{
//  Handle paint events.
...
VOID TransEllipseWin3 :: Print(VOID)
{
//  print drawing.
...
VOID TransEllipseWin3 :: PrintToFile(VOID)
{
//  print drawing.
...
VOID TransEllipseWin3 :: GetDimensions(char *p0, char *p1,
...
VOID TransEllipseWin3 :: TransferEllipsePar(float p1, float p2, float p3,
                                           float p4, float p5, float p6, char *info)
{
    counter++;
    if (counter > 3)
        counter = 3;
...
VOID TransEllipseWin3 :: FourFrames(VOID)
{
    NumberOfFrames = 4;
    Invalidate2();
}

VOID TransEllipseWin3 :: SingleFrame(VOID)
{
    NumberOfFrames = 1;
    Invalidate2();
}

VOID TransEllipseWin3 :: DrawEllipses(VOID)
{
    int j, x, y;
    ...
    {
       if (fabs(r13[i]) < 1.0e-10) r13[i] = (float)0.0;
       float sinch1 = r13[i];
       ...
       GraMove(x0x[i]+xlx[i],y0x[i]-int((float)ylx[i]*r13[i]));  // conjugated diameters
       GraDraw(x0x[i]-xlx[i],y0x[i]+int((float)ylx[i]*r13[i]));
       GraMove(x0x[i]+int((float)xlx[i]*r13[i]),y0x[i]-ylx[i]);
       GraDraw(x0x[i]-int((float)xlx[i]*r13[i]),y0x[i]+ylx[i]);

       if (fabs(r24[i]) < 1.0e-10) r24[i] = (float)0.0;
       float sinch2 = r24[i];
       ...
       GraMove(x0y[i]+xly[i],y0y[i]-int((float)yly[i]*r24[i]));  // conjugated diameters
       GraDraw(x0y[i]-xly[i],y0y[i]+int((float)yly[i]*r24[i]));
       GraMove(x0y[i]+int((float)xly[i]*r24[i]),y0y[i]-yly[i]);
       GraDraw(x0y[i]-int((float)xly[i]*r24[i]),y0y[i]+yly[i]);
    }
}

VOID TransEllipseWin3 :: DrawCaptions(VOID)
{
    char string[80];
    int  i, fs;
...
//  Write Captions
    ...
    for (i=0; i<=imax; i++)
    {  
       float chix = float(180.0 / 3.14159) * float(asin(r13[i]));
       sprintf(string,String(IDS_CH1_FORM),chix, r13[i]);
       PutString(x0x[i]-len, y0x[i] - 2 * len + 3 * fs, string);

       float chiy = float(180.0 / 3.14159) * float(asin(r24[i]));
       sprintf(string,String(IDS_CH2_FORM),chiy, r24[i]);
       PutString(x0y[i]-len, y0y[i] - 2 * len + 3 * fs, string);

       float aux  = (float)1.0 - r13[i] * r13[i];
       if (aux < 0.0) aux = (float)0.0;
       float epsx = x[i] * xp[i] * (float)sqrt(aux);
       sprintf(string,String(IDS_EP1_FORM),epsx,dim0,dim2);
       PutString(x0x[i]-len, y0x[i] - 2 * len + 5 * fs, string);

       aux  = (float)1.0 - r24[i] * r24[i];
       if (aux < 0.0) aux = (float)0.0;
       float epsy = y[i] * yp[i] * (float)sqrt(aux);
       sprintf(string,String(IDS_EP2_FORM),epsy,dim1,dim3);
       PutString(x0y[i]-len, y0y[i] - 2 * len + 5 * fs, string);

       sprintf(string,String(IDS_XXM2_FORM),
                      xscal[i], dim0, xpscal[i], dim2);
       PutString(x0x[i]-len, y0x[i] + 2 * len - 7 * fs, string);

       sprintf(string,String(IDS_YYM2_FORM),
                      yscal[i], dim1, ypscal[i], dim3);
...
VOID TransEllipseWin3 :: Dsplay(int w)
{
    CreatePens(w);
    DrawFrame();
    DrawEllipses();
...
VOID TransEllipseWin3 :: Erase(VOID)
{
    counter = -1;
    Invalidate2();
}
