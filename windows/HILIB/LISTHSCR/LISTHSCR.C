...
BOOL FAR PASCAL FInitListboxExtents(HWND hList)
{
    ...
    hMem=LocalAlloc(LMEM_MOVEABLE | LMEM_ZEROINIT,
         (CBALLOCUNIT + sizeof(WORD))<<1 );
...
WORD FAR PASCAL WAddExtentEntry(HWND hList, LPSTR psz)
{
    ...
    if (cExtentEntries==cExtentEntriesMax)
    {
        ...
        cExtentEntriesMax += (WORD)CALLOCUNITS;
        pw=(WORD *)LocalLock(hMem);
    ...
    else
    {
        ...
        for (iRev=(WORD)(cExtentEntries+1); iRev > i; iRev--)
            pw[iRev]=pw[iRev-1];
...
WORD FAR PASCAL WRemoveExtentEntry(HWND hList, WORD iSel)
{
    ...
    SendMessage(hList, LB_GETTEXT, iSel, (LONG)(LPSTR)pch);
...
BOOL FReAllocExtentList(HANDLE hMem, BOOL fGrow)
{
    ...
    wSize=(WORD)LocalSize((HLOCAL)hMem);
    wSize+=(fGrow) ?  (WORD)((int)CBALLOCUNIT) : (WORD)(-(int)CBALLOCUNIT);
...
WORD WGetListboxStringExtent(HWND hList, LPSTR psz)
{
    ...
    wExtent = (WORD)((WORD)size.cx + (WORD)tm.tmAveCharWidth);
