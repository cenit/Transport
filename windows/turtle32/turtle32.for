C
C     TURTLE AS WINDOWS-3 DDL
C     =======================
C
C =======================================================================
C
      INTEGER*2 FUNCTION TURTLEF(CMDLINE)
C
...
C
C     MODIFIED FOR MICROSOFT FORTRAN V5.1 WINDOWS-DLL FOR IBM-PC
C     BY URS ROHRER (PSI) IN AUGUST 1992.
C     TWO OLD (MORE THAN 10 YEARS OLD) BUGS FIXED IN THIS VERSION:
C     1.) INTEGER FUNCTION NU(TYPE) HAS GIVEN A UNDEFINED RETURN VALUE
C         FOR TYPE CODE 60 (DECAY). THIS LED TO A FLUSH IN SUBROUTINE
C         SETUP (MESSAGE: DATA OVERFLOW, NEL = ..., I = ...).
C     2.) SUBROUTINE HISCAP(IOPT,NUN,KFL,ZH,NFL) HAS BEEN CALLED FROM
C         SUBROUTINES OHIST AND HEDING (SECTION 'HISTOGRAMS OF PARTICLES
C         LOST') WITH PARAMETER KFL = 0 INSTEAD OF 1. FOR DECAY ENABLED THIS
C         LED TO AN ADDRESSING ERROR IN THE HISCAP-LINE 'PART(2) = KPART(KFL)'
C
...
C UNIT2        INTEGER   FILE UNIT TO WHICH RAYS ARE WRITTEN
C ----------------------------------------------------------------------
...
C FIRST1       LOGICAL FLAG IN BKOLL  (FIRST TIME TRUE, THEN FALSE)
C FIRST2       LOGICAL FLAG IN DINLAN (FIRST TIME TRUE, THEN FALSE)
C FIRST3       LOGICAL FLAG IN PKOLL  (FIRST TIME TRUE, THEN FALSE)
C FIRST4       LOGICAL FLAG IN SPSHLR (FIRST TIME TRUE, THEN FALSE)
C FIRST5       LOGICAL FLAG IN RAYRUN (FIRST TIME TRUE, THEN FALSE)
C ---------------------------------------------------------------------
C PERC                FOR TRANSMISSON OF WORK ALREADY DONE
C IPERC        INTEGER*2 PERCENTAGE OF WORK ALREADY DONE
C ---------------------------------------------------------------------
C STOP                FOR SUBROUTINES FINISH AND RAYRUN
C STFLAG       LOGICAL FLAG TO SIGNAL TERMINATION
C ----------------------------------------------------------------------
C ----------------------------------------------------------------------
C
     ...
     A                PAR1, PAR2
      ...
      CHARACTER*4     LABSUP
      COMMON /BLOC51/ IOCASE(5), IOUNIT(5), IONUM, UNIT1, UNIT2
      INTEGER UNIT1, UNIT2
      COMMON /BLOC60/ NCT
      COMMON /BLOC61/ DBQ, DBQQ, OLDG, DPP0, P0SV
      COMMON /BLOC62/ IUNIT3RD, NUMLIM, ZZ
C
      COMMON /FIRST/ FIRST1, FIRST2, FIRST3, FIRST4, FIRST5, SNTNL
      LOGICAL FIRST1, FIRST2, FIRST3, FIRST4, FIRST5, SNTNL
      COMMON /NNN/ NUC
      ...
      COMMON /STOP/ STFLAG
      LOGICAL STFLAG
      COMMON /EXITFLAG/ NNFLAG, NNFLAG2
      COMMON /hal/ halton, ipn(10) ,iseries(10)
      LOGICAL HALTON
C
C      INTEGER*2 SEEDVAL, IHR, IMIN, ISEC, I100TH
      INTEGER*4 ISEED(1)
      ...
      HALTON = .FALSE.
C
C     GET LENGTH OF TRANSFERRED COMMAND LINE
C
      DIR = CMDLINE
      DO 20 J = 1, 80
...
C
  999 CONTINUE
      TURTLEF = NNFLAG - 1
      IF (UNIT1 .NE. 0) CLOSE(UNIT=UNIT1)
      IF (UNIT2 .NE. 0) CLOSE(UNIT=UNIT2)
      CLOSE(UNIT=NIN)
      CLOSE(UNIT=NOUT)
...
C
      NNFLAG2 = NNFLAG
C
C      WRITE(MESS,1000) NNFLAG
C 1000 FORMAT('MENU IFLAG =',I3)
C      CALL MYMESSAGEBOX(MESS//CHAR(0))
C
      RETURN
      END

      SUBROUTINE SSTOP(TEXT)
C
C     SIMULATE STOP STATEMENT
C
      CHARACTER*(*) TEXT
C
      COMMON /EXITFLAG/ NNFLAG, NNFLAG2
      COMMON /BLOC3/ NIN, NOUT
C
      WRITE(NOUT,'(1X,A)') TEXT
      NNFLAG = 2  ! ERROR IN FOR001.DAT INPUT
C
      RETURN
      END
C
C     GET PERCENTAGE OF PARTICLES ALREADY RUN THROUGH THE SYSTEM
C
      INTEGER*2 FUNCTION PERCENTF(PAR)
C
      INTEGER*2 PAR
C
      COMMON /PERC/ IPERC
...
C
      ...
      END
C
C     SEND MESSAGE TO RAYRUN TO FINISH
C
      SUBROUTINE FINISHF
C
      COMMON /STOP/ STFLAG
...
C
      CALL RANDOM_NUMBER(RND)
      COST = 2.0 * RND - 1.0
      SINT = SQRT (MAX(1.E-12,1.0 - COST ** 2))
      GAMCOS = GAMMA * COST
C
...
C
C     READ IN STORING FILE AND DELETE IT
C
      IF (IOCASE(L) .GE. 0) THEN
       CALL UNIFIL(FILNAM,IOUNIT(L))
       OPEN(UNIT=IOUNIT(L),FILE=DIR(1:LDIR)//FILNAM,STATUS='OLD',
     X      ERR=6)
...
C
    6 OPEN(UNIT=IOUNIT(L),FILE=DIR(1:LDIR)//FILNAM,STATUS='NEW',
     X     ERR=7)
      GO TO 5
    7 OPEN(UNIT=IOUNIT(L),FILE=DIR(1:LDIR)//FILNAM,STATUS='UNKNOWN')
      REWIND(UNIT=IOUNIT(L))
    5 DO 1 I=1,LIMNO
    1 IV(I)=0.0
      DO 3 I=1,NHISTS
...
C
 ...
 9500 FORMAT(/' Histogram No',I3,' of ',A4,' lost')
      ...
      CALL HISCAP (4, N, 1, 0.0, 0)
      GO TO 700
  600 CALL HISCAP (5, N, 1, 0.0, 0)
      CALL HISCAP (6, NA, 1, 0.0, 0)
  700 CONTINUE
...
C
      ...
      END IF
      CALL RANDOM_NUMBER(RND)
      TH = ASN * Y2 * SQRT(-2. * LOG(MAX(1.E-12,1. - RND)))
      CALL RANDOM_NUMBER(RND)
      ARG = 6.283185 * RND
...
C
      DE = 0.0
      CALL RANDOM_NUMBER(RND)
      IF (DELT .NE. 0.0) DE = DELT / 5.8 * DINLAN(RND)
      DE= DE + ENLOSS
      IF (DE .EQ. 0.0) RETURN
      E = P**2 / ((1. + GAMMA) * SM)
      K = LFLAG + 1
...
C
  600 ND   = LTHY (K)
      ...
      DMIN = YSTRT(K) / FACT
      DD = YBIN(K) / FACT
      CALL HISCAP (5, NUNIT, 1, 0.0, 0)
      CALL HISCAP (6, NUND, 1, 0.0, 0)
...
C
      ...
      IF (OUTREC) THEN
       IF (ABS(XR).GT.ODIMX .OR. ABS(YR).GT.ODIMY) THEN
        ...
        NEL = NEL + 2
        IF (IDATA(I) .NE. 3)
     1  CALL SSTOP('Use of 16. 176. card sequence not correct')
        RETURN 1
       ENDIF
      ELSE
...
C
      IF (STFLAG) THEN
       IF (MOD(NCT, 100) .EQ. 0) NORAYS = NCT
      ENDIF
C
...
C
      ...
      REAL        UORIG(12), ZL(4), WORK(30)
      CHARACTER*4 NORIG(12), IRAYS, LTAGS
      INTEGER     ICC(4)
...
C
      ...
      OPEN(UNIT=UNIT1,FILE=DIR(1:LDIR)//FILNAM,STATUS='OLD',
     X     ERR=2222)
      GO TO 100
 2222 WRITE(NOUT,1112) FILNAM
 1112 FORMAT(' File ',A10,' with beam input data not found')
      CALL MENU(1)
      RETURN
C
...
C     MISALIGNMENTS
C
  280 CALL SSTOP('This program will not do misalignments.')
      RETURN
C
C      REPEAT
C
...
C
  360 IDATA(I) = TYPE
      ...
      CALL SSTOP('Index of multipole may not exceed 20.')
      RETURN
 3601 MPA = .TRUE.
      IDATA(I+1) = - NM
...
C
   ...
   49  CONTINUE
       ...
       OPEN(UNIT=UNIT2,FILE=DIR(1:LDIR)//FILNAM,FORM='FORMATTED',
     X      ACCESS='APPEND')
C
...
C
      ELSE IF (NM.EQ.176) THEN
       ...
       ELSE IF (DATA(I+2).GT.0.0 .AND. DATA(I+3).GT.0.0) THEN
        ...
        ODIMY  = DATA(I+3) * UNIT(3)
       ELSE
        CALL SSTOP('wrong parameters on 16. 176. xx yy card.')
        RETURN
       ENDIF
C
...
C
      ELSE IF (NM.EQ.179) THEN
       ...
       ELSE
        ...
        ELSE
         CALL SSTOP('more than three 16. 179. x. y. ; cards')
         RETURN
        ENDIF
       ENDIF
      RETURN
...
C
      ELSE IF (NM .EQ. 191) THEN
       ...
       OPEN(UNIT=IUNIT3RD,FILE=DIR(1:LDIR)//FILNAM,FORM='FORMATTED',
     X      STATUS='UNKNOWN')
...
C
      WRITE(NOUT,*) ' '
      WRITE(NOUT,1000)
 1000 FORMAT('Turtle32.DLL computational part for Turtle',
     X     ' Framework, compiled by U. Rohrer (PSI), 25-Mar-2008')
      WRITE(NOUT,*) ' '
C
      RETURN
      END
